...
- **FPS が落ちる**: 粒子数（Q/A）を下げる。エフェクトセット（1/2）を OFF にする。パターン（3/4/5/6）を OFF にする。
...
- パターン（3/4/5/6 キー）は背景要素として負荷が低いですが、必要に応じて OFF にすると軽量化できます。
...
### シナリオ4: 背景パターンを使った演出
1. **3** を押して Milky Way パターンをON（星空背景）
2. **6** を押して Water Surface パターンをON（海底コースティクス）
3. **Q** で粒子数を増やして前景に追加
4. **Z** でブルームを加えて幻想的な雰囲気を作る
